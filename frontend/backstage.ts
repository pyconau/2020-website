...
const TIME_COMPACT = {weekday: 'short', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit', timeZoneName: 'short', hour12: false}
const TIME_COMPACT_ADL = {...TIME_COMPACT, timeZone: 'Australia/Adelaide'}
const TIME_HRMIN = {hour: '2-digit', minute: '2-digit', timeZoneName: 'short', hour12: false}
const TIME_HRMIN_ADL = {...TIME_HRMIN, timeZone: 'Australia/Adelaide'}

function clockTick(output: HTMLElement){
  const now = DateTime.local()
  output.innerText = now.setZone('Australia/Adelaide').toLocaleString(TIME_COMPACT_ADL)
...
export default function(){
  ...
  if (localStorage.getItem('backstage') !== null){
    ...
    // add backstage bar
    const bar = backstageBar.content.firstElementChild.cloneNode(true) as HTMLElement
    ...
    document.querySelectorAll('[data-reltime]').forEach((el: HTMLElement) => {
      ...
      console.log(el, dt)
      const rtt = reltimeSpan.content.firstElementChild.cloneNode(true) as HTMLElement
      rtt.querySelector('.reltime-fixed').innerHTML = `${dt.toLocaleString(TIME_HRMIN_ADL)} (${dt.toLocaleString(TIME_HRMIN)})`
